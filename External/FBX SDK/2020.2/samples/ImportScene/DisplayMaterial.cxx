    ...
    if (lMaterialCount > 0)
    {
        ...
        for (int lCount = 0; lCount < lMaterialCount; lCount ++)
        {
            ...
            if(lImplementation)
            {
                ...
                FBXSDK_printf("            Language: %s\n", lImplementation->Language.Get().Buffer());
                FBXSDK_printf("            LanguageVersion: %s\n", lImplementation->LanguageVersion.Get().Buffer());
                FBXSDK_printf("            RenderName: %s\n", lImplementation->RenderName.Buffer());
                FBXSDK_printf("            RenderAPI: %s\n", lImplementation->RenderAPI.Get().Buffer());
                FBXSDK_printf("            RenderAPIVersion: %s\n", lImplementation->RenderAPIVersion.Get().Buffer());
